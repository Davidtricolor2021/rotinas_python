...
# v8.2: 24/02/25 implementação de outras possibilidades nos redimensionamentos com e sem filtro de movimentos.
#*******************************************************************************************************************#
...
def selecionar_dados_camera(conexao_bco=None, loja=None, equipamento=None, camera=None):
    ...
    coordenadas_movimento=None
    flg_movimento = None
    id_subprocesso_vet = []

    cursor = conexao_bco.cursor() # prepara bco para operações na tabela tb_esclacao_cliente

    sql = "SELECT dimensao_resizer, fps_camera, coordenadas_mascara, coordenadas_movimento, id_loja_equip_cam, flg_movimento "
    ...
    if ret_query is not None:
        ...
        coordenadas_movimento = ret_query['coordenadas_movimento']
    ...
    return dimensao_resizer, fps_camera, coordenadas_mascara, coordenadas_movimento, id_subprocesso_vet, flg_movimento
...
class ler_video_redimensionar:
    ...
    def exec_lvr(self, caminho=None):
        ...
        while True:
            ...
            # Salvar o frame de saida
            frame_out = frame.copy()

            # Aplica a máscara somente se ela existir
...
class RotinaPrincipal:
    ...
    def looprotinaprincipal(self, caminho_origem=None, caminho_destino=None, caminho_mascara=None, nome_thread=None):
        ...
        for arquivo in caminho_origem: # encontrou arquivo i: 'NVR_ch1_main_20221220114000_20221220115000.dav'
            ...
            dimensao_resizer, fps_camera, coordenadas_mascara, coordenadas_movimento, id_subprocesso_vet, flg_movimento = selecionar_dados_camera(
            ...
            flg_acao_A = roi_A_tmp['flg_acao']
            roi_B_tmp = json.loads(coordenadas_movimento)
            roi_B = roi_B_tmp['coordenadas'] # coordenadas para a máscara de movimento especifico (ex. parme)
            flg_acao_B = roi_B_tmp['flg_acao']
            tm_mask = json.loads(dimensao_resizer) # tamanho w x h para a máscara

            # Configuração para NÃO filtrar movimentos
            if flg_movimento==0:
                # Configuração para NÃO filtrar movimentos e flg_acao_A == True (Montar a mascara)
                if flg_acao_A: # CONDIÇÃO 1
                ...
                # Configuração para NÃO filtrar movimentos e flg_acao_A == False (Não Montar a mascara)
                else: # CONDIÇÃO 2
                    tempo_inicial = time.time()
                    ...
                    # executar a classe video-redim
                    video_inferencia = ler_video_redimensionar_COMPLETO(escala_conversao=tm_mask).start(arg=arquivo)
            ...
            # Configuração para FILTRAR movimentos
            elif flg_movimento == 1:
                if flg_acao_A == False and flg_acao_B == True: # Essa condiçao para salvar vídeo pela área de coordenandas_movimento (Parme) # CONDIÇÃO 3
                    tempo_inicial = time.time()
                    ...
                    roi_corners = np.array(roi_B, dtype=np.int32)
                    channel_count = 3
                    ...
                    while True:
                        ...
                        if contador_lote == 10:
                            contador_lote = 0
                            if detected_motion:
                                #cv2.imshow("Frame original a ser salvo", frame_out)
                                #cv2.imshow("Máscara de movimento", fg_mask)
                                cv2.waitKey(1)
                ...
                elif flg_acao_A == False and flg_acao_B == False: # Essa condiçao redimensiona o video somente com movimento, salvando toda a area do video, só que sem ascrecentar as coordenadas_mascara, e tbm não faz uso de coordenadas_movimentos # CONDIÇÃO 4
                    tempo_inicial = time.time()
                    ...
                    # executar a classe video-redim
                    video_inferencia = ler_video_redimensionar(escala_conversao=tm_mask, backSub=backSub).start(arg=arquivo)
                ...
                elif flg_acao_A: # Essa condiçao redimensiona o video somente com movimento, salvando toda a area do video, ascrecentanto as coordenadas_mascara idependente de cooordenadas_movimento ser true ou false, essa é a mesma condição que o Else faz na versão antierior do código # CONDIÇÃO 5
                    tempo_inicial = time.time()
